"""
Enhanced Prompt Construction Service for Adaptive Control System that handles
the construction of system messages, domain context, agent capabilities
and other contextual information. Enhanced with dynamic MCP tool discovery integration.

This service is now more focused on dynamically discovering and integrating 
MCP-compatible tools into prompts.
"""
import logging
...
class PromptConstructionService:
    """
    Enhanced service that handles the construction of prompts with contextual information,
    ...
    Enhanced with MCP tool discovery integration to dynamically discover available tools
    and include them in the prompt construction process.
    """
    ...
    def __init__(self, agent_registry: Optional[Dict[str, Any]] = None, 
        ...
        """
        Initialize the enhanced prompt construction service.
    ...
    async def construct_contextual_messages(
        self,
        user_input: str,
        ...
        include_mcp_discovered_tools: bool = True
    ) -> List[Dict[str, str]]:
        """
        Construct messages with contextual information, including dynamically discovered MCP tools.
        ...
        Args:
            ...
            include_mcp_discovered_tools: Whether to include dynamically discovered MCP tools
        ...
        if include_agent_capabilities and registry_to_use:
            agent_capabilities = await self._format_agent_capabilities(registry_to_use)
            if agent_capabilities:
                system_content += agent_capabilities
        
        # Include system service capabilities if requested
        if include_system_services:
            system_service_capabilities = await self._format_system_service_capabilities()
            if system_service_capabilities:
                system_content += system_service_capabilities
        
        # Include MCP-discovered tools if requested and MCP client is available
        if include_mcp_discovered_tools and self.mcp_client_service:
            mcp_tools_info = await self._discover_and_format_mcp_tools()
            if mcp_tools_info:
                system_content += mcp_tools_info
    ...
    async def _format_agent_capabilities(self, agent_registry: Dict[str, Any]) -> str:
        """
    ...
    async def _format_system_service_capabilities(self) -> str:
        """
        ...
        if self.mcp_client_service:
            capabilities_str += await self._format_mcp_client_capabilities()
        
        return capabilities_str
    
    async def _format_mcp_client_capabilities(self) -> str:
        """
    ...
    async def _discover_and_format_mcp_tools(self) -> str:
        """
        Discover and format MCP-compatible tools available in the system.
        
        Returns:
            Formatted MCP tools string
        """
        if not self.mcp_client_service:
            return ""
        
        try:
            # Discover tools from connected external agents
            tools_info = await self._get_all_available_mcp_tools()
            if not tools_info:
                return ""
            
            mcp_tools_str = "\\n[MCP_DISCOVERED_TOOLS]\\n"
            mcp_tools_str += "Available MCP-discovered tools:\\n"
            
            # Add connected external agent tools
            if tools_info.get("connected_external_agent_tools"):
                mcp_tools_str += "Connected External Agent Tools:\\n"
                for tool_name, tool_info in tools_info["connected_external_agent_tools"].items():
                    if isinstance(tool_info, dict):
                        desc = tool_info.get("description", "No description")
                        mcp_tools_str += f"  - {tool_name}: {desc}\\n"
                    else:
                        mcp_tools_str += f"  - {tool_name}: {tool_info}\\n"
            
            # Add system services via MCP
            if tools_info.get("system_services"):
                mcp_tools_str += "System Services via MCP:\\n"
                for service_name, service_info in tools_info["system_services"].items():
                    if isinstance(service_info, dict) and "description" in service_info:
                        mcp_tools_str += f"  - {service_name}: {service_info['description']}\\n"
                    else:
                        mcp_tools_str += f"  - {service_name}: Available\\n"
            
            mcp_tools_str += "[/MCP_DISCOVERED_TOOLS]\\n"
            return mcp_tools_str
    ...
    async def _get_all_available_mcp_tools(self) -> Dict[str, Any]:
        """
        Get a comprehensive list of all available tools in the system via MCP.
        
        Returns:
            Dict containing all available MCP tools organized by type
        """
        tools_info = {
            "system_services": {},
            "mcp_tools": {},
